...
import com.example.android.cityvisitor.databinding.FavMonumentListFragmentBinding

private const val LOCATION_PERMISSION_REQUEST = 1

private const val LOCATION_PERMISSION = "android.permission.ACCESS_FINE_LOCATION"

class FavMonumentListFragment : Fragment() {


    private val viewModel: FavMonumentListViewModel by lazy {
        ViewModelProvider(this).get(FavMonumentListViewModel::class.java)
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val binding = FavMonumentListFragmentBinding.inflate(inflater)
        ...
        val adapter = FavMonumentListAdapter(FavMonumentClickListener { monument ->
            val destination = Uri.parse(monument.website)
            startActivity(Intent(Intent.ACTION_VIEW, destination))
        }, UnfavouriteMonumentClickListener { monument ->
            viewModel.removeMonumentFromFavourite(monument)
            Snackbar.make(
                binding.root,
                "Usunięto z listy ulubionych "+monument.name,
        ...
        binding.favMonumentList.adapter = adapter
        ...
        viewModel.regionList.observe(viewLifecycleOwner, object: Observer<List<String>> {
            override fun onChanged(data: List<String>?) {
                data ?: return
                val chipGroup = binding.favRegionList
